...
def create_master_pass_group(groups, delimeter="_x_"):
    ...
    for group in groups:
        ...
        for item in groupItems:
            ...
            transformItems = [i for i in item.itemGraph('xfrmCore').reverse() if
                              i.type in ('scale', 'rotation', 'translation')]
            for transformItem in transformItems:
                for channel in transformItem.channels():
                    channels.add(channel)

        for action in [i for i in groups[0].itemGraph('itemGroups').forward() if
                       i.type == lx.symbol.a_ACTIONCLIP]:
...
def combine(master_group, groups, channels, max_depth, depth=0, passname_parts=[], delimeter="_"):
    ...
    if depth < max_depth:
        passes = [i for i in groups[0].itemGraph('itemGroups').forward() if
                  i.type == lx.symbol.a_ACTIONCLIP]
        ...
        for p in passes:
            if p.actionClip.Enabled():
                ...
                combine(master_group, subgroups, channels, max_depth, depth + 1,
                        passname_parts + [p.name])

                p.actionClip.SetActive(0)

    elif depth == max_depth:
        layer_name = delimeter.join(passname_parts)
        lx.eval('group.layer group:{%s} name:{%s} transfer:false grpType:pass' % (
        master_group.name, layer_name))
        for c in channels:
            try:
